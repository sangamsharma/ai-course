...
import { getCorporateModule, getCorporateLesson } from "@/data/corporate-modules";
import { cn } from "@/lib/utils";
import { ArrowLeft, ArrowRight, Clock, CheckCircle2, Lightbulb } from "lucide-react";

export default async function CorporateLessonPage({
  params,
}: {
  ...
  const lesson = getCorporateLesson(moduleSlug, lessonSlug);
  if (!lesson) notFound();
    ...
    <div className="max-w-3xl mx-auto px-6 py-12">
      ...
      {lesson.exercises.length > 0 && (
        <AnimatedEntry delay={0.2}>
          <div className="mt-10">
            ...
            <LessonExerciseTracker exercises={lesson.exercises} lessonId={lesson.id} />
          </div>
        </AnimatedEntry>
      )}
      ...
      <AnimatedEntry delay={0.4}>
        <div className="mt-12 flex items-center justify-between gap-4">
          {prevLesson ? (
            <Link
              href={`/corporate/${mod.slug}/${prevLesson.slug}`}
          ...
          {nextLesson ? (
            <Link
              href={`/corporate/${mod.slug}/${nextLesson.slug}`}
              className={cn(buttonVariants({ variant: "default" }))}
            >
              Next Lesson <ArrowRight className="ml-2 h-4 w-4" />
            </Link>
          ) : (
            <Link href="/corporate" className={cn(buttonVariants({ variant: "default" }))}>
              Finish Track <CheckCircle2 className="ml-2 h-4 w-4" />
            </Link>
          )}
        </div>
      </AnimatedEntry>
